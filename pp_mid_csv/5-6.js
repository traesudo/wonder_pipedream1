...
class UploadCompanyInfoTask {
    ...
    async runTask($) {
        ...
        //step2: 生成本地csv文件
        var selfAdd= await this.getCounter($);
        console.log("selfAdd Res:",selfAdd )
        const remoteFilePath = this.generateRemoteFilePath();
        const csvFilePath = this.generateCSV(companies,selfAdd);

        console.log(remoteFilePath)
        var number = this.updateCounter(selfAdd);
    ...
    async updateCounter($,pp_counter){
    ...
    async getCounter($){
        ...
        try{
            ...
            console.log("getConter Error:", err)
        }
    }


    generateCSV(data,selfAdd) {
        console.log(data)
        ...
        console.log('selfAdd的值',selfAdd)
        const remoteFilePath = this.generateRemoteFilePath();
        fs.writeFileSync(remoteFilePath,csvContent);
        //console.log(csvContent)
        console.log(remoteFilePath)
        return remoteFilePath,selfAdd;
    }
